        ...
        init: function () { //初始化监听
           var webSocket = new WebSocket("ws://127.0.0.1:8999");
            ...
            webSocket.onmessage = function(message) {
                ...
                //         console.log(message);
                //         $.get('class/doAction.php?action=add_admin_msg', {
                //             from: message.from,
                //             adminGroup: message.to,
                //             to: message.gid
                //         }, function (res) {
                //             // var data = eval('(' + res + ')');
                //             // if (data.code == 0) {
                    ...
                    var options = {
                        ...
                        success: function (respData) {
                            if (respData.data.groupid) {
                                $.get('class/doAction.php?action=commitGroupInfo', {
                                    groupIdx: respData.data.groupid,
                                    groupName: data.groupName,
                                    des: data.des,
                                    number: data.number,
            ...
            return data = {
                ...
                callback: function (ele) {
                    ...
                    layer.prompt({title: '请输入分组名，并确认', formType: 0, value: groupName}, function (mygroupName, index) {
                        if (mygroupName) {
                            $.get('class/doAction.php?action=editGroupName', {
